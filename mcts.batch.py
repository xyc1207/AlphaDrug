...
if __name__ == '__main__':
    ...
    parser.add_argument('--pdb_id', type=str, default=None, help='pdb id to be infer')
    ...
    with open("finished_list") as fr:
        finished_pdb = [e.strip() for e in fr]
        finished_pdb = set(finished_pdb)
    ...
    if args.source == 'new':
        ...
        if args.pdb_id is not None:
            for idx in range(len(test_pdblist)):
                if args.pdb_id == test_pdblist[idx]:
                    args.k = idx
    ...
    print(test_pdblist[args.k])
    if test_pdblist[args.k] in finished_pdb:
        print(test_pdblist[args.k], " is decoded")
        exit()
